...
### Вывод:
MPI ускоряет перемножение матриц равномерно, увеличивая процентный прирост по скорости с увеличением числа элементов матриц, на размере 3000 прирост достигает 8 раз. На маленьких размерах  ускорения нет из-за накладных расходов на создание потоков и конкуренции за память.
*Конкретно в 3 лабе решил использовать другую IDE для работы, скорее всего она является причиной прироста в производительности такого.
